...
def mine_binary(din='../data/compas', dout='../data/compas', froot='compas', max_cardinality=2,
    ...
    fout = os.path.join(dout, '%s%s.out' % (prefix, froot))
    flabel = os.path.join(dout, '%s%s.label' % (prefix, froot))
    ...
    if (minor):
        import minority
        fminor = os.path.join(dout, '%s%s.minor' % (prefix, froot))
        print('computing', fminor)
        minority.compute_minority(froot='%s' % froot, dir=dout)
